...
public class MapUnit extends DM2R2kObject {
    ...
    @DMFXOBinding("@events") @DM2LcfBinding(81)
    ...
    protected Object dm2AddField(Field f) {
        if (f.getName().equals("layer0"))
            return layer0 = new BlobR2kStruct(context, R2kUtil.supplyBlank(20 * 15 * 2, (byte) 0));
        if (f.getName().equals("layer1"))
            return layer1 = new BlobR2kStruct(context, R2kUtil.supplyBlank(20 * 15 * 2, (byte) 0));
    ...
    protected IRIO dm2AddIVar(String sym) {
        if (sym.equals("@data"))
            return map = new IRIOFixedUser(context, "Table", new RubyTable(3, 20, 15, 2, new int[] {0, 0}).innerBytes);
        if (sym.equals("@events"))
            return events = new DM2SparseArrayH<Event>(context, () -> new Event(context));
    ...
    private void decLmuData() {
        ...
        layer0 = new BlobR2kStruct(context, new byte[width.i * height.i * 2]);
        layer1 = new BlobR2kStruct(context, new byte[width.i * height.i * 2]);
