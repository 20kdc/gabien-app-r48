...
public class OSStrHashMapSchemaElement extends SchemaElement {
    ...
    public UIElement buildHoldingEditor(final RubyIO target, ISchemaHost launcher, final SchemaPath path) {
        if (AppMain.osSHESEDB == null)
            AppMain.launchDialog(TXDB.get("This is basically useless without a locmaps.txt file. Please prepare one by going into RXP mode, System Tools, and pressing 'Retrieve all object strings'."));
    ...
    public static void tryInitOSSHESEDB() {
        ...
        try {
            inp = GaBIEn.getFile(PathUtils.autoDetectWindows(AppMain.rootPath + "locmaps.txt"));
            if (inp == null)
                return;
