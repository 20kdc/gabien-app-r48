...
public class RMAnimRootPanel extends UIPanel {
    ...
    public void prepareFramesetCache() {
        ...
        if (nameA.length() != 0) {
            framesetCacheA[15] = GaBIEn.getImage(AppMain.rootPath + "Graphics/Animations/" + nameA + ".png", 0, 0, 0);
            framesetCacheAMirror[15] = mirrorFrameset(framesetCacheA[15]);
        }
        if (nameB.length() != 0) {
            framesetCacheB[15] = GaBIEn.getImage(AppMain.rootPath + "Graphics/Animations/" + nameB + ".png", 0, 0, 0);
